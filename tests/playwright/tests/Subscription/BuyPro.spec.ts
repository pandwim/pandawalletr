...
test('Buy PRO', async ({ page }) => {
  ...
  await expect(page.getByText('PANDAWALLETR Pro', { exact: true })).toBeVisible();
  await expect(page.getByText('A PANDAWALLETR Pro subscription')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('A PANDAWALLETR Pro subscription includes advanced wallet functionality, offering a suite of tools for managing your cryptocurrency.');
  ...
  await expect(page.getByText('Inside PANDAWALLETR Pro –')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('Inside PANDAWALLETR Pro – Dashboard');
  await expect(page.locator('#react-portal-modal-container')).toContainText('In the Dashboard you will have access to wallet analysis. This feature will benefit professional users who use PANDAWALLETR in their daily business.');
  ...
  await expect(page.locator('#react-portal-modal-container')).toContainText('With multi-send feature you can send funds up to 255 addresses at once. PANDAWALLETR Pro saves time and simplifies the process of mass transactions.');
  ...
  await expect(page.getByText('PANDAWALLETR ProPANDAWALLETR Pro\'s')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('PANDAWALLETR ProPANDAWALLETR Pro\'s subscription comes with an extended wallet feature, offering a toolset for crypto management.');
  ...
  await expect(page.getByRole('heading', { name: 'PANDAWALLETR Pro' })).toBeVisible();
  await expect(page.getByText('PANDAWALLETR Pro\'s subscription')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('PANDAWALLETR Pro\'s subscription comes with an extended wallet feature, offering a toolset for crypto management.');
