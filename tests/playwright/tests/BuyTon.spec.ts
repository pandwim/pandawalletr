...
test('Buy TON on main screen', async ({ page }) => {
  ...
  await expect(page.getByText('You are opening an external app not operated by PANDAWALLETR.Terms of usePrivacy')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('You are opening an external app not operated by PANDAWALLETR.');
