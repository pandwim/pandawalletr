...
test('Welcome screen', async ({ page }) => {
  ...
  await expect(page.getByRole('heading')).toContainText('Welcome to PANDAWALLETR');
  ...
  await expect(page.getByText('PANDAWALLETR stores your')).toBeVisible();
  await expect(page.locator('#root')).toContainText('PANDAWALLETR stores your cryptographic keys on your device. All trades are executed via decentralized protocols so that your crypto never ends up in the hands of centralized exchanges.');
