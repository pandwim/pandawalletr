...
const useIsPANDAWALLETRMobileInstalled = () => {
    return useQuery(
        ['isPANDAWALLETRMobileInstalled'],
        async () => {
...
export const RedirectToPANDAWALLETRMobile: FC<{
    isOpen: boolean;
    ...
    const { data: isPANDAWALLETRMobileInstalled } = useIsPANDAWALLETRMobileInstalled();
    const notifyError = useToast();

    useEffect(() => {
        if (isOpen && !isPANDAWALLETRMobileInstalled && unsupportedLinkError) {
            notifyError(unsupportedLinkError);
        }
    }, [isOpen, isPANDAWALLETRMobileInstalled, unsupportedLinkError, notifyError]);

    return (
        <HideOnReview>
            <ReactPortal position="first" wrapperId="body">
                <AnimatePresence>
                    {isOpen && isPANDAWALLETRMobileInstalled && (
                        <Wrapper
